...
pwd 명령어 이외에도 
```bash
# 명령어를 실행할 필요 없이, 시스템이 가지고 있는 PWD 환경 변수 값을 출력하는 방식
# 특징: pwd 명령어보다 실행 속도가 미세하게 빠르며, 스크립트 작성 시 자주 쓰입니다.
echo $PWD

# 현재 디렉토리(.)의 절대 경로를 ls 명령어의 -d 옵션(디렉토리 자체 정보만 출력)과 조합해 확인합니다.
ls -d .
```
...
이 외 
```bash
# 파일을 새로 만들면서 터미널에서 바로 내용을 작성하고 싶을 때 사용합니다.
# 위 명령어를 입력하면 줄바꿈이 되며 입력 대기 상태가 됩니다.
# 원하는 내용을 작성한 후 Ctrl + D를 누르면 저장되고 빠져나옵니다.
cat > test.txt

# > (덮어쓰기/새로 만들기)
echo "Hello World" > test.txt

# 빈 파일만 즉시 만들기 (명령어 없이 연산자만 사용)
> test.txt
```
</details>
...
`-r` 옵션을 붙이면, 지정한 폴더뿐만 아니라 그 폴더 속에 숨어있는 모든 파일과 하위 폴더들까지 한꺼번에 몽땅 복사해 줍니다.
...
# 절대 경로 (/Users/...): 전체 주소 (내가 어디에 있든 무조건 정확한 그 위치를 가리킴)
# 상대 경로 (./... 또는 ../...): 내 위치 기준 길 안내 (현재 내가 있는 폴더에 따라 대상 위치가 바뀜)
```
...
<details>
<summary><strong>2-6. 컨테이너 리소스 사용량 확인</strong></summary>

실행 중인 Docker 컨테이너가 사용하고 있는 CPU, 메모리, 네트워크 및 디스크 자원을 확인하기 위해 `docker stats` 명령어를 실행했습니다.

```bash
docker stats
```

`docker stats`는 현재 실행 중인 컨테이너의 리소스 사용량을 실시간으로 확인하는 명령어입니다.

명령어를 실행하면 컨테이너의 상태가 일정한 간격으로 계속 갱신됩니다.
...
| `NAME` | 컨테이너 이름 |
| `CPU %` | 컨테이너의 CPU 사용률 |
| `MEM USAGE / LIMIT` | 현재 메모리 사용량과 사용 가능한 최대 메모리 |
| `MEM %` | 컨테이너의 메모리 사용 비율 |
| `NET I/O` | 컨테이너가 송수신한 네트워크 데이터 |
| `BLOCK I/O` | 컨테이너가 디스크에서 읽고 쓴 데이터 |
| `PIDS` | 컨테이너 내부에서 실행 중인 프로세스 수 |

`docker stats`는 실행 중인 컨테이너만 표시합니다.

종료된 컨테이너는 CPU나 메모리를 사용하지 않기 때문에 목록에 나타나지 않습니다.

### 실시간 확인 종료

`docker stats` 화면은 계속 갱신되므로 다음 키를 눌러 종료했습니다.

```text
Ctrl + C
```

`Ctrl + C`는 Docker 컨테이너를 종료하는 명령이 아닙니다.

현재 터미널에서 실행 중인 `docker stats` 명령만 종료하며, 컨테이너는 계속 실행됩니다.
...
실행 결과를 통해 각 컨테이너가 사용하는 CPU, 메모리, 네트워크, 디스크 및 프로세스 수를 확인했습니다.

<!-- 컨테이너 리소스 사용량 확인 이미지 삽입 위치 -->
...
```text
포트 매핑이 필요한 이유 : 컨테이너 내부와 외부(내 컴퓨터)는 완전히 격리된 별개의 네트워크를 사용하기 때문

쉽게 비유하자면:
- Docker 컨테이너: 아파트 단지 안의 특정 동/호수 (내부)
- 포트 매핑: 외부 방문자가 찾아올 수 있도록 아파트 경비실(외부 Port)과 해당 집(컨테이너 Port)을 이어주는 벨

포트 매핑이 없으면 일어나는 일
Docker 컨테이너 안에서 웹 서버를 80번 포트로 띄웠다고 가정

포트 매핑을 안 했을 때 (-p 옵션 없이 실행):
컨테이너 내부에서는 80번 포트로 서비스가 잘 돌고 있습니다.
하지만 내 컴퓨터(호스트)의 브라우저에서 localhost:80으로 접속하려고 하면 연결이 거부됩니다. 외부에서 컨테이너 내부 네트워크로 들어가는 문이 닫혀있기 때문입니다.

포트 매핑을 했을 때 (-p 8080:80 실행):
"내 컴퓨터의 8080번 포트로 들어오는 요청을 컨테이너의 80번 포트로 연결해라"라는 통로를 만들어 줍니다.
이제 브라우저에서 localhost:8080에 접속하면 컨테이너 내부의 웹 서버로 정상 접속됩니다.
```
...
- Git: 내 컴퓨터에서 코드 변경 이력을 기록하고 관리하는 소프트웨어(도구)
- GitHub: Git으로 기록한 코드를 인터넷에 올려서 백업하고 팀원과 공유하는 웹 서비스(공간)
